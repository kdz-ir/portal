...
export class DormitoryListPageComponent implements AfterViewInit {
  ...
  async onDownloadExport() {
    ...
    const cols = [{ name: 'نام و نام خانوادگی' }, { name: 'امکانات' }, { name: 'جنسیت' }, { name: 'شهر' }, { name: 'کد ملی' }, { name: 'شماره پرداخت' }];
    const row = this.data.filter(c=>!isEmpty(c.refId)).map(d => ([d.profile.name + ' ' + d.profile.family, d.type, d.profile.sex, d.profile.city, d.nationalCode, d.refId]));
